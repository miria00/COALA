'''
final plot generation for EDU with selective smoothing and x-axis limits
...
# This is for edu dataset:
FOLDERS_TO_PROCESS = [
    "/home/miria/COALA/wandb_data_csv_competition2_train/DPO",
    "/home/miria/COALA/wandb_data_csv_competition2_eval/DPO",
    "/home/miria/COALA/wandb_data_csv_competition1_eval/ORPO",
    "/home/miria/COALA/wandb_data_csv_competition1_train/ORPO",
    "/home/miria/COALA/wandb_data_csv_coala4"
]

# Output directory for plots (current directory)
OUTPUT_DIR = "/home/miria/COALA/plots_edu/"
...
def get_legend_sort_order(label):
    ...
    Order: LLaMA-8B, LLaMA-8B-SFT, Mistral7B, Mistral-7B-SFT, Dolphin2.6-7B, Dolphin2.6-7B-SFT
    """
    order_map = {
        'LLaMA-8B': 0,
        'LLaMA-8B-SFT': 1,
...
def should_apply_smoothing(folder_path):
    ...
    Only applies to the two specified training folders.
    """
...
def filter_and_plot_runs(folder_path):
    ...
    for csv_path in csv_files:
        try:
            ...
            if "DPO" in folder_path or "ORPO" in folder_path:
                # Divide all metric columns by 100 for DPO/ORPO folders
    ...
    if all_test_dfs:
        ...
        if "coala4" in folder_path.lower():
            # COALA4: 5 ticks evenly spaced from 0.48 to 0.68
            ax.yaxis.set_major_formatter(FormatStrFormatter('%.2f'))
            yticks = np.linspace(0.48, 0.68, 5)  # Creates [0.48, 0.47, 0.54, 0.61, 0.68]
            ax.set_yticks(yticks)
            ax.set_ylim(bottom=0.48, top=0.68)  # Set both bottom and top limits
        elif "DPO" in folder_path or "ORPO" in folder_path:
            # DPO/ORPO folders: use 2 decimal places for consistency
            ax.yaxis.set_major_formatter(FormatStrFormatter('%.2f'))
            # Set y-axis maximum to 1.00 for DPO/ORPO folders
            ax.set_ylim(top=1.00)
        
        # Set x-axis limits based on folder type
        if "coala4" in folder_path.lower():
        ...
        if new_handles and new_labels:
            ...
            # Add smoothing info to legend title for smoothed folders
            if should_apply_smoothing(folder_path):
